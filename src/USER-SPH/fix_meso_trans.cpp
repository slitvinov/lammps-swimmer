...
FixMesoTrans::FixMesoTrans(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->e_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
        "fix meso/trans command requires atom_style with both energy and density");

  if ((atom->fb_flag != 1))
    error->all(FLERR,
        "fix meso/trans command requires atom_style with fb (background force) support");
...
void FixMesoTrans::initial_integrate(int vflag) {
...
void FixMesoTrans::final_integrate() {
