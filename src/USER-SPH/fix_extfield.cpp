...
FixExtField::FixExtField(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 15) error->all(FLERR,"Illegal fix_extfield command");
  ...
  if (nfileevery) {
    if (narg != 16) error->all(FLERR,"Illegal fix_extfield command");
    MPI_Comm_rank(world,&me);
    if (me == 0) {
      fp = fopen(arg[15],"w");
      if (fp == NULL) {
        char str[128];
        sprintf(str,"Cannot open fix_extfield file %s",arg[15]);
        error->one(FLERR,str);
      }
    }
  }
  ...
  if (seed <= 0) error->all(FLERR,"Invalid random number seed in fix_extfield command");
  if (electronic_specific_heat <= 0.0)
    error->all(FLERR,"fix_extfield electronic_specific_heat must be > 0.0");
  if (electronic_density <= 0.0)
    error->all(FLERR,"fix_extfield electronic_density must be > 0.0");
  if (electronic_thermal_conductivity < 0.0)
    error->all(FLERR,"fix_extfield electronic_thermal_conductivity must be >= 0.0");
  if (gamma_p <= 0.0) error->all(FLERR,"fix_extfield gamma_p must be > 0.0");
  if (gamma_s < 0.0) error->all(FLERR,"fix_extfield gamma_s must be >= 0.0");
  if (v_0 < 0.0) error->all(FLERR,"fix_extfield v_0 must be >= 0.0");
...
void FixExtField::setup(int vflag)
{
...
void FixExtField::post_force_setup(int vflag)
{
  double **f = atom->f;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixExtField::post_force_respa_setup(int vflag, int ilevel, int iloop)
{
...
void FixExtField::end_of_step()
{
  ...
  if (stability_criterion < 0.0) {
    ...
    if (num_inner_timesteps > 1000000)
      error->warning(FLERR,"Too many inner timesteps in fix_extfield",0);
  }
...
void FixExtField::grow_arrays(int ngrow)
{
...
void FixExtField::write_restart(FILE *fp)
{
  double *rlist;
...
void FixExtField::restart(char *buf)
{
  int n = 0;
...
int FixExtField::pack_restart(int i, double *buf)
{
  buf[0] = 4;
  buf[1] = flangevin[i][0];
  buf[2] = flangevin[i][1];
...
void FixExtField::unpack_restart(int nlocal, int nth)
{
...
int FixExtField::maxsize_restart()
{
  return 4;
}
...
int FixExtField::size_restart(int nlocal)
{
  return 4;
}
