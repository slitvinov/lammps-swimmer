...
#include "math.h"
#include "stdlib.h"
#include "bond_harmonic_swimmer.h"
#include "atom.h"
#include "neighbor.h"
#include "domain.h"
#include "comm.h"
...
BondHarmonicSwimmer::~BondHarmonicSwimmer()
{
  if (allocated) {
...
void BondHarmonicSwimmer::allocate()
{
  allocated = 1;
...
double BondHarmonicSwimmer::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondHarmonicSwimmer::write_restart(FILE *fp)
{
...
void BondHarmonicSwimmer::read_restart(FILE *fp)
{
  allocate();
...
void BondHarmonicSwimmer::write_data(FILE *fp)
{
...
double BondHarmonicSwimmer::single(int type, double rsq, int i, int j,
				 double &fforce)
{
  double r = sqrt(rsq);
  double dr = r - r0[type];
